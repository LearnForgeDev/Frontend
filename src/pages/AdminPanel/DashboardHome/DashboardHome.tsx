...
import { useActiveSchoolRequests } from "../../../hooks/useActiveSchoolRequests";

const DashboardHome: React.FC = () => {
  const services = serviceRegistry.getAll();
  const widgetServices = services.filter((service) =>
    Boolean(service.DashboardWidget),
  );
  const { requests, isLoading: isLoadingRequests } = useActiveSchoolRequests();

  const hasAnyContent = widgetServices.length > 0 || requests.length > 0;
    ...
    >
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
          <Box sx={{ display: "inline-flex", color: "var(--admin-primary)" }}>
            {service.icon}
          </Box>
          <Typography
            component="h3"
            sx={{ margin: 0, fontSize: "1rem", color: "var(--admin-text)" }}
          >
            {service.name}
          </Typography>
        </Box>
